...
Referral contract should be a claimable contract
  ...
  function init(CSCStorage storage self,  address[] members, uint[] ppms, address refer, address owner) internal {
    ...
    require(sum >= MILLION - 1 && sum < MILLION + 1, "The total should add up to a million");
  }

  function addSplit(CSCStorage storage self, Split newSplit, address owner) internal {
    require(newSplit.ppm > 0, "No zero splits");
  ...
  function payAll(CSCStorage storage self) internal {
    for(uint index = 0; index < self.splits.length; index++) {
      ...
      if(value > 0 ) {
        require(self.splits[index].to.call.gas(GASLIMIT).value(value)(), "Transfer must succeed");
  ...
  function claimFor(CSCStorage storage self, address user) internal {
    require(self.isClaimable, "Contract must be in claimable mode");
    uint sum = getClaimableBalanceFor(self, user);
    uint splitIndex = self.userSplit[user];
    self.lastUserClaim[user] = self.deposits.length;
    if(sum > 0) {
      require(self.splits[splitIndex].to.call.gas(GASLIMIT).value(sum)(), "Tranfer must succeed");
  ...
  function transfer(CSCStorage storage self, address newTo, uint ppmToTransfer, address newOwner, address owner, address toTransfer) internal {
    ...
    require(splitIndex > 0, "Developer fee isn't transferrable for now");
    require(ppmToTransfer > 0, "Can't transfer zero");
    require(getClaimableBalanceFor(self, toTransfer) == 0, "Can't transfer when you have ether to be claimed");
    require(getClaimableBalanceFor(self, newTo) == 0, "Can't transfer to someone who has ether to be claimed");
    // neither user can have a pending balance to use transfer
    require(self.splits[splitIndex].to == toTransfer, "Original destination must match param");
    require(self.splits[splitIndex].ppm >= ppmToTransfer, "Must have enough to transfer");
    require(self.ownership[owner][splitIndex] >= ppmToTransfer, "Must own enough of that split to transfer");
